...
resource "azurerm_snapshot" "infinite_os_disk_snapshot" {
  ...
  resource_group_name = var.resource_group_name
  create_option       = "Copy"             
  source_uri          = var.source_os_disk_uri
  tags = {
    "purpose" = "cloning"
...
resource "azurerm_snapshot" "infinite_data_disk_snapshot" {
  ...
  for_each             = {for k,v in var.data_disks : k => v if v["data_disk_create_option"]=="Copy"}
  name                = "${each.value.data_disk_name}-snapshot"
  location            = var.location
  resource_group_name = var.resource_group_name
  create_option       = "Copy"
...
resource "azurerm_managed_disk" "infinite_data_disk" {
  ...
  source_resource_id = (each.value.data_disk_create_option) == "Empty" ? null : azurerm_snapshot.infinite_data_disk_snapshot[each.key].id

  depends_on = [
    azurerm_snapshot.infinite_data_disk_snapshot
  ]
}
...
resource "null_resource" "cooloff" {
  ...
  depends_on = [
    azurerm_virtual_machine_extension.infinite_extension_windows, 
    azurerm_virtual_machine_extension.infinite_extension_linux_hostname,
    azurerm_virtual_machine_extension.infinite_extension_linux_hostname_timezone,
  ]
}
